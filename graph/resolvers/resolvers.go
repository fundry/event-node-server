...
func (r *mutationResolver) UpdateCurrentStream(stream *model.Stream) (*model.Stream, error) {
	_, err := r.DB.Model(stream).Where("id = ?", stream.ID).Update()
	return stream, err
}
...
func (r *mutationResolver) GetStreamById(id int) (*model.Stream, error) {
	stream := model.Stream{} // de-reference from here <-
	err := r.DB.Model(&stream).Where("id = ?", id).First()
	return &stream, err
}
