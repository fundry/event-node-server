...
func (r *mutationResolver) CreateEvent(ctx context.Context, input model.CreateEvent, userID int) (*model.Event, error) {
    ...
    if len(input.Summary) > 100 {
        return nil, errors.New("input should be less than 100")
    }

    if _, err := http.Get(Envs["EMAIL_FUNCTION_ENDPOINT"]); err != nil {
        return nil, errors.New("couldn't send email")
    }
...
func (r *mutationResolver) UploadSingleFile(ctx context.Context, req model.UploadFile) (*model.File, error) {
    file := model.File{
        ID:        time.Now().Nanosecond(),
        File:      req.File,
        FileURI:   req.FileURI,
    ...
    // run gcloud upload here

    return &file, nil
}

func (r *mutationResolver) UploadMultipleFiles(ctx context.Context, req []*model.UploadFile) ([]*model.File, error) {
    files := model.File{
        ID:        time.Now().Nanosecond(),
        Timestamp: time.Now(),
    }
